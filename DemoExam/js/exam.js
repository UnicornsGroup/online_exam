...
window.addEventListener('DOMContentLoaded', () => {
    ...
    async function compileBrandedDocumentForPDFDownload(e) {
        ...
        // Dynamically extract the appConfig matrix to enforce dynamic branding updates
        const config = window.appConfig;
        const institutionLogo = config?.logoPath ? `../${config.logoPath}` : 'https://via.placeholder.com/150?text=Logo';
        const institutionNameHeader = config?.institutionName || "Assessment Portal";
        const institutionAddress = config?.contactDetails?.address || "";
        const institutionWebsite = config?.contactDetails?.website || "";
        const institutionMobile = config?.contactDetails?.mobile || "";
        const institutionWhatsapp = config?.contactDetails?.whatsapp || "";
        ...
        try {
            ...
            <body>
                <div class="paper-matrix">
                    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #1e3a8a; padding-bottom: 10px; margin-bottom: 15px;">
                        <div style="width: 15%;"><img src="${institutionLogo}" style="max-height: 80px; width: auto;" onerror="this.src='https://via.placeholder.com/150?text=Logo'"></div>
                        <div style="width: 82%; text-align: right;">
                            <h1 style="font-size: 24px; font-weight: 900; color: #1e3a8a; margin: 0; text-transform: uppercase; font-family: sans-serif;">${institutionNameHeader}</h1>
                            <p style="font-size: 11px; color: #374151; margin: 2px 0 0 0; line-height: 1.4; font-family: sans-serif;">
                                Address: ${institutionAddress}<br>
                                Website: <b>${institutionWebsite}</b> | Mobile: ${institutionMobile} | WhatsApp: ${institutionWhatsapp}
                            </p>
                        </div>
                    </div>
